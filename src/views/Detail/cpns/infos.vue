  ...
  <div class="infos">
    ...
    <div class="tags">
      <template v-for="(item,index) in topInfos.houseTags" :key="index">
        <span class="item" :style="{color:item.tagText.color,backgroundColor:item.tagText.background.color}" v-if="item.tagText">
          {{ item.tagText.text }}
        </span>
      </template>
    </div>
    <div class="comment extra">
      ...
      <div class="right">
        <div class="count">{{ topInfos.commentBrief.totalCount }}条评论
          <van-icon name="arrow" />
        </div>
      </div>
    </div>
